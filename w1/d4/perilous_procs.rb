...
def filter_out(arr, &p)
    output = []
    arr.each { |el| output << el if !p.call(el) }
    output
end
...
def every?(arr, &p)
    arr.each { |el| return false if !p.call(el)}
    true
end
...
def first_index(arr, &p)
    arr.each_with_index { |el, i| return i if p.call(el)}
    nil
end
...
def xnor_select(arr, p1, p2)
    arr.select { |el| (p1.call(el) && p2.call(el)) || (!p1.call(el) && !p2.call(el)) }
end
...
def multi_map(arr, num = 1, &p)
    count = 0
    while count < num
        arr.map! { |el| p.call(el) }
...
def chain_map(val, arr_procs)
    arr_procs.inject(val) { |acc, p| p.call(acc) }
end
...
def proc_suffix(sentence, hash)
    ...
    (0...sentence.split(" ").length).each do |idx|
        temp = sentence.split(" ")[idx]
...
def proctition_platinum(arr, *procs)
    # iterate through hash, then iterate through that proc 
        # the elements that pass true through that proc will be shoveled into that hash[key] as value
    hash = Hash.new([])
    (1..procs.length).each { |key| hash[key] = []}

    hash.keys.each do |proc_idx|
        arr.each do |ele|
            # iterate through hash.values arrays to see if ele exists
            go = true
            hash.values.each do |subarray|
                go = false if subarray.include?(ele)
            end
            if procs[proc_idx - 1].call(ele) && go == true
                hash[proc_idx] << ele
            end
        end
    end
